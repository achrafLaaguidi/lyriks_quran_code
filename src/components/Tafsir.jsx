...
const TafsirCard = ({ tafsir, i, t, isPlaying, activeTafsir, handlePauseClick, handlePlayClick }) => (
  ...
  >
    ...
    <div className="flex-1 flex flex-row justify-between items-center">
      <img className="md:w-20 w-16 md:h-20 h-14 rounded-lg " src={Tafsir} alt={tafsir?.name} />
      <div className="flex-1 flex flex-col justify-center mx-3">
        <p className="md:text-xl text-base font-bold text-white ">{tafsir?.name}</p>
        <p className="md:text-base text-sm text-gray-300 mt-1">{t('Sourat') + ' ' + t('Number')} {tafsir?.sura_id}</p>
      </div>
    </div>
    <PlayPause
      isPlaying={isPlaying}
